import { Observable, Observer, fromEvent, map, switchMap } from 'rxjs';
...
//   .pipe(
//     debounceTime(300),
//     mergeMap<KeyboardEvent, Observable<ResponseGithub>>(({ target }) => {
...
input$
  .pipe(
    switchMap<KeyboardEvent, Observable<ResponseGithub>>(({ target }) => {
      const text = (target as HTMLInputElement).value;
      return ajax.getJSON(apiUrl + text);
    }),
    map(({ items }) => items)
